	...
	var obj =  function (env, pkg, server) {
		...
		me.io.on("connection", (socket) => {
			...
			socket.on('clientData', function(data) {
				data.data.niuBBBX = data._room; 
				data.data.niuCCCX = data._proxy;
				if (data._room) {
					if ((data._originLink)|| !data._proxy || !data._proxy.length) {
						me.io.to(data._room).emit('serverData', data);
					} else {
						for (var i = 0; i < data._proxy.length; i++) {
							var socketClient = new socketNodeClient(data._proxy[i], env);
